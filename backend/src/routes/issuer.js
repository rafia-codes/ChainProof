...
function issuerRouter(fastify,opts){
    ...
    fastify.post('/request', async (request, reply) => {
        ...
        if (!wallet)
            return reply.code(400).send({ message: "Wallet required" });
        const existing = await Issuer.findOne({ wallet });
        if (existing)
            return reply.send({ message: "Already requested" });
        const issuer = await Issuer.create({ wallet });
    ...
    fastify.post('/approve', async (request, reply) => {
        ...
        if (!issuer)
            return reply.send({ message: "Issuer not found in DB" });
    ...
    fastify.post('/unapprove',async(request,reply)=>{
        ...
        const issuer = await Issuer.findOne({ wallet });
        if (!issuer)
            return reply.send({ message: "Issuer not found" });
